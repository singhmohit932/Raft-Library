// Test Cluster for writing tests for Raft.
package raft
...
// NewCluster creates a new test Cluster, initialized with n servers connected
// to each other.
...
// Shutdown shuts down all the servers in the Cluster and waits for them to
// stop running.
...
func (h *Cluster) CrashPeer(id int) {
	tlog("Crash %d", id)
	h.DisconnectPeer(id)
	h.alive[id] = false
...
func (h *Cluster) RestartPeer(id int) {
	if h.alive[id] {
...
// are made.
func (h *Cluster) PeerDropCallsAfterN(id int, n int) {
...
func (h *Cluster) PeerDontDropCalls(id int) {
...
func (h *Cluster) CheckNoLeader() {
	for i := 0; i < h.n; i++ {
		if h.connected[i] {
			_, _, isLeader := h.cluster[i].rf.Report()
			if isLeader {
...
// log index.
func (h *Cluster) CheckCommitted(cmd int) (nc int, index int) {
	h.t.Helper()
	h.mu.Lock()
	defer h.mu.Unlock()
...
// servers.
func (h *Cluster) CheckCommittedN(cmd int, n int) {
	h.t.Helper()
...
func (h *Cluster) CheckNotCommitted(cmd int) {
	h.t.Helper()
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *Cluster) SubmitToServer(serverId int, cmd any) int {
...
func (h *Cluster) collectCommits(i int) {
